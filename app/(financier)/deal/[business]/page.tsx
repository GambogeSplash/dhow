    ...
    <div>
      ...
      <div className="mt-6 grid gap-8 lg:grid-cols-[1fr_1.1fr]">
        <section>
          ...
          <div className="mt-4 rounded-[var(--radius-card)] border border-brass/40 bg-surface p-5">
            ...
            {facility ? (
              <div className="rise mt-4 flex items-start gap-3 rounded-[var(--radius-card)] bg-brass-tint px-4 py-3">
                <div className="min-w-0">
                  <p className="font-medium text-brass-deep">
                    {aed(facility.amountAed)} {facility.repaid ? "repaid" : "disbursed to"}{" "}
                    {facility.repaid ? "" : borrower.name}
                  </p>
                  <p className="mt-0.5 text-sm text-ink-3">
                    Settled on Polygon
                    {facility.explorerUrl && facility.txHash && (
                      <>
                        {" · "}
                        <a
                          href={facility.explorerUrl}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="tnum font-mono text-teal-deep underline decoration-teal/30 underline-offset-2 hover:decoration-teal"
                        >
                          {shortHash(facility.txHash)} ↗
                        </a>
                      </>
                    )}
                  </p>
                  <Link
                    href="/portfolio"
                    className="mt-1 inline-block text-sm text-brass-deep underline underline-offset-2"
                  >
                    View in Portfolio →
                  </Link>
                </div>
              ...
              <p className="mt-3 text-sm text-ink-3">
                Fee 1.5% on draw, repaid from the borrower&apos;s next settlement. Risk carried by{" "}
                Creek Capital, not Dhow.
              </p>
            )}
